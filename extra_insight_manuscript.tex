...
\begin{abstract}
 ...
 Recently,
  whole genome sequencing data sets for tens of thousands of human individuals
  have enabled characterization of mutations having large selective effect, with selection coefficients exceeding $1$\%.
%
However, these methods have so far been applied to protein-coding genes only, focusing on protein-truncating variants.
%
  Here we describe a 
  new method, called ExtRaINSIGHT for analysing mutations of large effect in noncoding as well as coding regions of the human
  genome.
%
  ExtRaINSIGHT estimates the prevalance of strong purifying selection
  by  measuring the depletion for rare (minor allele frequency $<0.1$\%) variants  in a target set of genomic sites relative to nearby a nearby set of neutrally evolving sites
  after controlling for local variation in mutation rate and neighbor-dependent mutation rates.
%  
We show using simulations that, above a minimal threshold, the ExtRaINSIGHT score $\lambda$
  is highly predictive of the site-specific selection coefficient against heterozygous mutations $s_h$.
 %
 Applying these methods to $\sim$75,000 whole genome sequences from the gnomAD project,
%check number  
  we find that features in and around protein-coding genes, including splice sites and untranslated regions
  ExtRaINSIGHT scores are generally concordant with previous results based on protein-truncating variants.
  % say something more?
%  indicating $s_h \approx 0.03$ for highly disruptive mutations and average..... $s_h$.....
 % Neuronal genes (and...?) are enriched for strong selection, whereas.....
%
  Among noncoding elements, microRNAs (stem?) appear to be under very strong selection, on average,
  whereas structural RNAs and lincRNAs display considerably reduced evidence.
  % check
  Interestingly,  ultra-conserved elements and human accelerated elements display only moderate evidence of selection 
  despite their extreme conservation over millions of years of evolution.
  % something about genetic load result
  %
  Our model detects
Apparently owing to an atypical mutation pattern in the upstream regions of genes, 
...
Understanding the impact of new mutations on fitness has has been a major focus of evolutionary genetics for nearly a century \cite{FISH22,HALD27,HALD37}. Together with genetic drift and natural selection, mutation is one of three primary evolutionary forces, and it is the only one that can generate new genetic variation.  In this way, mutation is the fuel that enables natural selection to do its work.  Because fitness effects exist on a continuum, the general problem is to characterize the distribution of fitness effects (DFE) of new mutations.  Estimating the DFE is central in addressing a wide variety of fundamental problems in evolutionary genetics, including understanding the genetic architecture of complex traits, the emergence of recombination and sex, the behavior of populations maintained at small population sizes (as with domesticated animals and plants), and the effects of mutational load \cite{EYREKEIG07}.
...
For this reason, efforts to characterize the DFE in recent years have focused on the use of statistical modeling, population genetic theory, and large-scale DNA sequence data.  In humans, these statistical methods are essentially the only practical means for characterizing the DFE, and in all organisms, they have the advantage of providing access to a collection of naturally occurring mutations of both small and large effect.
...
Nevertheless, these strategies have some crucial limitations.  In particular, patterns of variation are strongly influenced by demographic history, especially by population bottlenecks, expansions, and substructure, so that careful demographic modeling is required to isolate the effects of selection.  In addition, the population panels that are typically available---consisting of hundreds to a few thousand individuals---are informative about only a relatively narrow slice of the DFE.  For example, strong purifying selection (such that $-s$ is much larger than $\sim$1\%) will tend to hold variants to such low frequencies in the population that they will not be reliably detectable in these panels, whereas weak purifying selection (such that $-s$ is on the order of $10^{-4}$ or less in humans) will be indistinguishable from random genetic drift.  Thus, only in approximately the range $-10^{-2} < s < -10^{-4}$  is there any hope of accurately measuring the strength of purifying selection.
...
[biobank, GNOMAD, what else....?] \cite{LEKETAL16}
...
although, as clarified by Fuller et al. \cite{FULLETAL19}, these measures are strictly informative only about the strength of selection against heterozygous mutations.
and are of limited value in ..... [direct estimation of $s_h$]  owing to length dependency and ....

(cite also Petrovski, Utah group, others)   \cite{PETRETAL13}

An approach more grounded in population genetic theory has been taken by Cassa et al.\ \cite{CASSETAL17} and Weghorn et al.\ \cite{WEGHETAL19}
who have shown that
