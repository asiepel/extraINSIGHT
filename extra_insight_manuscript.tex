...
48\% of sites were subject to ultra-selection, in comparison with 90\% of sites subject to weak or strong selection.

Coding sequences (CDS) showed reduced but still considerable purifying selection, with 
15\% of sites subject to ultraselection
and 62\% of sites subject to selection

In $3'$ UTRs, the fraction of ultraselected dropped considerably to 2.5\%, despite that 22\% of sites showed evidence of some type of selection
...
% show the ratio in the table?
Interestingly the ratio of $\lambda_s$ to $\rho$ varied considerably across regions
suggesting major differences in the relative proportions of weakly and strongly selected sites

In particular, in splice sites this ratio exceeded 0.5, suggesting that more than half of functional sites were ultraselected,
whereas it dropped to 0.24 in CDS and, further, to 0.12 in $3'$ UTRs.

These observations suggest dramatically different distributions of fitness effects even within the fraction of sites subject to selection in these genomic regions.
...
To see whether ExtRaINSIGHT was capable of distinguishing among protein-coding sequences experiencing different levels of selection against heterozygous loss-of-function variants
we compared it with the recently introduced loss-of-function observed/expected upper bound fraction (LOEUF) measure \cite{KARCETAL20}.

LOEUF is similarly based on rare variants
but it differs from ExtRaINSIGHT in several key ways:
it is computed separately for each gene
pools together all mutations predicted to result in loss of function of that gene (including nonsense mutations, mutations that disrupt splice sites, ......)
and uses a z-score transformation (check)

By contrast, $\lambda_s$ is computed for pooled collections of sites from coding or noncoding sites distributed across the genome
and represents a fraction of sites subject to ultraselection

To compare the two, we divided all genes [which set?]
into deciles by LOEUF score and ran ExtRaINSIGHT separate on the pooled coding sites corresponding to each decile.

Again, we computed $\rho$ values using INSIGHT together with the $\lambda_s$ values.

We found that both $\rho$ and $\lambda_s$ decreased monotonically with LOEUF decile
% would it make more sense to define the deciles in the reverse order?   higher score more conserved?
with $\lambda_s$ ranging from slighly less than 0.3 for the genes having the highest (?) LOEUF scores
to slightly more than 0 for the genes having the lowest LOEUF scores.
As expected, $\rho$ was considerably larger, ranging from just under 0.8 to about 0.45
...
range from about 10\% to 25\% for $\lambda_s$
